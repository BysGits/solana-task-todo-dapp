...
export function useTodo() {
    ...
    useEffect(() => {
        // Fetch user profile and its todo accounts
            ...
            if(program && publicKey && !transactionPending) {
                try {
                    ...
                    if(profileAccount) {
                        ...
                        setInitialized(true)
                        console.log("User profile : " + profilePda.toString());
    ...
    const initializeUser = async () => {
        // Check if program exists and wallet is connected
    ...
    const addTodo = async (e) => {
        ...
        if(program && publicKey && !transactionPending) {
            try {
                ...
                if(input) {
                    const tx = await program.methods.addTodo(input).accounts({
                        userProfile: profilePda,
                        todoAccount: todoPda,
                        authority: publicKey,
                        systemProgram: SystemProgram.programId,
    ...
    const removeTodo = async (todoPda, todoIdx) => {
        if(program && publicKey && !transactionPending) {
            try{
                ...
                await program.methods.deleteTodo(todoIdx).accounts({
                    userProfile: profilePda,
                    todoAccount: todoPda,
                    authority: publicKey,
                    systemProgram: SystemProgram.programId,
